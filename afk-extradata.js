  ...
  window.AFK_EXTRA = {
    ...
    mapName: function (id) {
      try {
        ...
        if (typeof SiegeV2 !== 'undefined' && SiegeV2.stages) {
          for (var sv = 0; sv < SiegeV2.stages.length; sv++) {
            var stage = SiegeV2.stages[sv];
            if (stage && stage.map === id) return stage.mapName || stage.name || id;
          }
        }
