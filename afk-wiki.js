...
(function () {
  ...
  var COMBAT_SECTIONS = [
    ...
    { t: '重擊 vs 爆擊（含魔法爆擊）', blocks: [
      { t: 'tbl', h: ['類型', '機率', '效果'], rows: [
        ...
        ['物理爆擊', '看近戰/遠程爆擊率（黑妖天賦+3%、爆擊精通+3%、麗人3件近戰+3%、疾風3件遠程+3%）', '這一擊<b>多打 50%</b>'],
        ['魔法爆擊', '獨立一套，看魔法爆擊率（主要靠智力、學徒3件+2%）', '該發魔法<b>多打 50%</b>'],
        ['會心一擊', '重擊＋爆擊同時觸發', '取最高傷害<b>再乘</b>爆擊倍率（最痛）'],
        ['擦傷', '差距太大、勉強蹭中', '傷害只剩一半、<b>不會爆擊</b>']
      ]},
      { t: 'p', p: '麗人套裝 5 件改為：<b>裝備近距離武器時攻擊速度 +20%</b>；遠距離武器與空手不會獲得這項加速。' }
    ]},
    ...
    { t: '怪打你時：迴避(ER)・傷害減免(DR)・受到傷害減少%', blocks: [
      ...
      { t: 'tbl', h: ['防禦層', '作用', '來源／規則'], rows: [
        ['① 迴避率（ER）', '有機率<b>整個閃掉（0 傷害）</b>', '主要靠<b>敏捷</b>＋增益/套裝（迴避提升+12、月光3件+5）。<b>非 1:1</b>：50 以內 1 點=1%、超過 50 每 5 點才+1%。一般只閃物理。⚠ 帶<b>笨重的鋼鐵石盾</b>時完全<b>不能靠迴避閃</b>（暗隱術的必閃不受影響）'],
  ...
  var TRIAL_50 = {
    ...
    elf: { n: '妖精 50 級試煉', npc: '迪嘉勒廷 ＠象牙塔（需等級 50）', req: '兩階段依序交付：①古代黑妖之秘笈 ×1 → ②密封的情報書 ×1', from: '古代黑妖之秘笈：巨大兵蟻 1%（風木領域：沙漠／螞蟻洞窟 1~2 樓）；密封的情報書：在「燃柳村·大洞穴隱遁者村莊地區」打「魔族暗殺團」必掉。魔族暗殺團是特殊怪、不常駐，只有你正卡在這一步（缺密封的情報書）時，在這張圖每打一隻怪才有 1% 機率讓這隻怪出現（一次只會有一隻）', rw: '完成階段任務後開放「魔族神殿」。最終試煉：在魔族神殿打「炎魔的惡魔」（Lv61）取得「炎魔之爪」（最終兌換階段必定掉落）×1 交付 → 一次領取全部獎勵：傳說「赤焰之弓」＋「赤焰之劍」＋「賽菲亞長靴」（每個角色僅能完成一次）' },
    mage: { n: '法師 50 級試煉', npc: '迪嘉勒廷 ＠象牙塔（需等級 50）', req: '一階段：交付 間諜報告書 ×1', from: '間諜報告書：在「燃柳村·大洞穴隱遁者村莊地區」打「魔族暗殺團」必掉。魔族暗殺團是特殊怪、不常駐，只有你正卡在這一步（缺間諜報告書）時，在這張圖每打一隻怪才有 1% 機率讓這隻怪出現（一次只會有一隻）', rw: '完成階段任務後開放「魔族神殿」。最終試煉：在魔族神殿打「炎魔的惡魔」（Lv61）取得「炎魔之眼」（最終兌換階段必定掉落）×1 交付 → 一次領取全部獎勵：傳說武器「瑪那水晶球」＋「瑪那長靴」（每個角色僅能完成一次）' },
  ...
  var state = { tab: 'equipbook', cls: 'knight', q: '', magicCls: 'all', magicChar: '', collMode: null, equipCls: 'all', equipSlot: 'all', equipRegion: 'all', equipPage: 0, relicRegion: 'all' };   // 預設分頁=分頁列第一個(收藏-裝備)。equipSlot 必須是 EQUIP_GROUPS 的 key 或 'all'(舊值 'wpn' 已無此分組→整頁空白)
  ...
  // 裝備列表改分頁後，搜尋不能再靠 renderEquip() 的第一頁 DOM。
  ...
  function renderDograce() {
    ...
    return note
      ...
      + wCard('每場怎麼決定勝負',
          wDesc('每場開賽前，每隻狗隨機抽一個「當日狀態」，再乘上該狗的波動係數、疊到基礎實力＝<b>該場戰力</b>；戰力越高勝率越高（賠率也越低）。')
          + wTbl(['當日狀態', '加成', '出現權重'], [
            ['🔥 極佳', '+2.4', '1'],
            ['😊 良好', '+1.1', '2'],
            ['😐 普通', '0', '3（最常見）'],
  ...
  var MERC_BUFF_SCOPE = [
    ['傭兵自己的增益<br>（聖結界／龍裔／狂怒／鐵衛／覺醒／加速／神聖武器…）', '聖結界 受傷 −30%、龍裔 −15%、狂怒 5/5 最多 −15%、鐵衛 3/5 −20%', '<b>只有他自己</b>'],
  ...
  var RIFT_SECTIONS = [
    ...
    { t: '🐍 同領域的狩獵區:底比斯與提卡爾', lines: [
      ...
      '<b>庫庫爾坎寶箱</b>:神廟地區怪掉「初級寶箱碎片(上/下半)」、深處怪掉「高級寶箱碎片」各 <b>5%</b>,上下半拿去<b>希培利亞村莊的巴特爾</b>接合成寶箱;雙頭目每次擊殺必掉高級寶箱與龜裂之核。',
  ...
  var ANTHARAS_SECTIONS = [
    ...
    { t: '助戰者：借別的角色來加持自己（最多 4 位）', lines: [
      ...
      '<table style="width:100%;border-collapse:collapse;margin:4px 0;font-size:13px;color:#cbd5e1;"><thead><tr><th style="text-align:left;padding:4px 6px;border-bottom:1px solid #475569;">職務</th><th style="text-align:left;padding:4px 6px;border-bottom:1px solid #475569;">給你的加成</th></tr></thead><tbody>'
        ...
        + '</tbody></table>',
      '<b>每次跟多魯嘉貝爾對話都會依來源角色的最新存檔刷新</b>；來源角色被刪除或在同一格重建，助戰會自動解除。<b>助戰者與傭兵互斥（雙向）</b>：已受僱為傭兵的角色不能當助戰者；反過來，<b>正擔任你助戰者的角色也不能被你招募成傭兵</b>（要先找多魯嘉貝爾解除助戰）。同一角色也不能同時擔任兩種職務；這條只看你目前這隻角色，不影響其他角色。'
    ]},
